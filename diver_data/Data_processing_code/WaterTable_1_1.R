...
ele <- readxl::read_excel("data-raw/soil_moisture_probe_and diver_details.xlsx",
                sheet = 1)
...
## archiving locally
file.copy(all.files1, "data-raw/Diver-Office_exported data copy/CSV/compensated", overwrite = TRUE)
...
str(all.files1); str(all.files2)

total1 = length(all.files1); total2 = length(all.files2)
# create progress bar
...
ggsave(file.path("figures/Pressure observed by Divers.jpeg"), plot = g0, height = 7, width = 12, units='in')
